...
class AuditLedgerService:
    ...
    def log_event(self, data: Dict) -> int:
        ...
        # In the temporary single-runtime architecture we don't have the
        # Debezium/Kafka projection pipeline, so keep the read model updated
        # synchronously to preserve the audit API contract.
        AuditLogRead.objects.using("read").update_or_create(
            id=log.id,
            defaults={
                "utilisateur_id": data["utilisateur_id"],
                "action": data["action"],
