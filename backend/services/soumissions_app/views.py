    ...
    def patch(self, request, soumission_id, *args, **kwargs):
        ...
        for field, value in request.data.items():
            if field in allowed_fields:
                setattr(soum, field, value)

        soum.save()
        return Response(SoumissionListSerializer(soum).data, status=status.HTTP_200_OK)

    def delete(self, request, soumission_id, *args, **kwargs):
        soum = get_object_or_404(Soumission, id_soumission=soumission_id)
        soum.delete()
...
class AppelOffreSoumissionsView(APIView):
    """
...
class OperateurSoumissionsView(APIView):
    """
    GET /operateurs-economiques/{operateur_id}/soumissions
    """

    def get(self, request, operateur_id, *args, **kwargs):
