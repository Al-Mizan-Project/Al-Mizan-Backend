...
def validate_document_ids(document_ids, id_operateur=None):
    """Return (all_valid: bool, missing_ids: list, ownership_errors: list). Degrades gracefully.
    If id_operateur is provided, also checks that all documents belong to that operator."""
    if not document_ids:
        return True, [], []
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if id_operateur is not None:
                for item in results:
                    doc_owner = item.get("id_operateur_economique")
                    if doc_owner is not None and doc_owner != id_operateur:
...
def fetch_documents_by_ids(document_ids):
    ...
    Returns a list of document dicts. Degrades gracefully."""
    if not document_ids:
        return []
    ids_param = ",".join(str(d) for d in document_ids)
    url = f"{settings.DOCUMENTS_SERVICE_URL}/api/documents/search/"
