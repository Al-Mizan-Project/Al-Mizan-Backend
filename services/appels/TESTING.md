...
### View OpenAPI Spec
```ash
curl http://localhost:18083/openapi.json | jq
```

### Check Database Tables
```ash
# Connect to shared PostgreSQL
docker exec -it almizan_shared_postgres psql -U appels_user -d appels_db
...
### Port 18083 already in use
```ash
# Change APPELS_NGINX_PORT in .env
APPELS_NGINX_PORT=18090
```
...
```ash
# View logs
docker compose logs -f appels_api
