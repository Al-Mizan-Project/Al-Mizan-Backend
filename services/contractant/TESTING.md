...
### View OpenAPI Spec
```ash
curl http://localhost:18082/openapi.json | jq
```

### Check Database Tables
```ash
# Connect to shared PostgreSQL
docker exec -it almizan_shared_postgres psql -U contractant_user -d contractant_db
...
```ash
# Change CONTRACTANT_NGINX_PORT in .env
CONTRACTANT_NGINX_PORT=18090
```
...
```ash
# View logs
docker compose logs -f contractant_api
